...
### Fixed
- Fixed `AttributeError: 'MessageChain' object has no attribute 'image'` error when forwarding images
- Now correctly appends Image components to the message chain using `chain.chain.append()` instead of the non-existent `chain.image()` method
